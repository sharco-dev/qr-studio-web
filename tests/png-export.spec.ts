...
describe('PNG export source', () => {
  it('export target is #export-root, not .editor-canvas__container', () => {
    const { root } = renderExportPreview();
    expectNoEditorUI(root!);
    const editorContainer = document.querySelector('.editor-canvas__container');
    expect(root).not.toBe(editorContainer);
  });
  ...
  it('exportAsPng receives #export-root element', async () => {
    const { root } = renderExportPreview();
    const { exportAsPng } = await import('../src/utils/export');
